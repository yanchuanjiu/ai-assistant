...
**token 类型区分**（传参错误是最高频问题）：

| 值类型 | 示例 | 传参规则 |
|--------|------|---------|
| node_token ✅ | `FalZwGDOkiqpbQkeAjGc8jaznMd` | 所有工具均接受 |
| space_id ❌ | `7618158120166034630`（纯数字） | 不能传给工具，只在内部使用 |
| 完整 URL ✅ | `https://...feishu.cn/wiki/FalZwGDO...` | 工具自动解析 |

`list_children` 返回的每行 `token=` 后的值就是 node_token，直接传给下一步工具。

**授权问题**：遇到飞书 API 权限/token 错误时**不要调用 `feishu_oauth_setup`**，系统自动续期并通过飞书消息通知用户。只有用户主动说"帮我重新授权飞书"时才调用 `feishu_oauth_setup(action="get_auth_url")`。
