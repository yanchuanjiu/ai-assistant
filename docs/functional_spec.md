...
2. 截断历史 `ToolMessage` 内容至 300 字符（节省 context）
3. 调用 `_build_system_prompt()` 动态组装 system prompt
4. 调用 `_select_tools_for_context()` 按消息关键词选择工具集合
5. `llm.bind_tools(selected_tools).invoke(messages)`
6. 记录到 `logs/llm.jsonl`（model/latency/tokens/tool_calls）
7. 检测火山云文本格式工具调用（`<|FunctionCallBegin|>[...]`），转换为标准 tool_calls
8. 检查最大迭代次数（`MAX_TOOL_ITERATIONS=10`），超限强制终止
9. 检查 `_check_user_interaction_needed()`，如需用户交互则提前终止

**文本格式工具调用解析**（火山云 Ark 特有）：
...
| 分类 | 触发关键词（部分） | 工具数量 |
|------|------------------|---------|
| `feishu_wiki` | 飞书、wiki、知识库、项目、章程、周报、里程碑 | 6 |
| `feishu_advanced` | 多维表格、bitable、任务、日历、表格 | 6 |
| `claude` | 迭代、开发、claude、代码、编写 | 5 |
| `dingtalk_mcp` | 钉钉、dingtalk、会议、纪要 | 33+ MCP + 4 pipeline |

**Token 节省**：无相关关键词时只传 7 个工具 schema，节省约 87% token。
...
- `list_children(wiki_token)` — 列出子页面
- `create_page(parent_token, title, content)` — 创建新页面（create docx → move 两步）
- `find_or_create(parent_token, title, content)` — 查找或创建（先搜索后创建）
- **约束**：创建新 wiki 节点需 user token 或 FEISHU_WIKI_ROOT_NODES 配置

#### `feishu_wiki_delete(node_token)`（v1.0.17 新增）
- 删除 wiki 节点
...
#### `feishu_spreadsheet(action, spreadsheet_token, ...)`
支持操作：
- `create` — 创建新电子表格
...
#### `feishu_task_create(title, due_date, assignee_id, ...)`
- 创建飞书任务
- 支持 due_date、assignee、description 字段

#### `manage_topics(action, chat_id, topic_name)`（v1.0.15 新增）
支持操作：
...
#### `get_recent_chat_context(thread_id, limit=10)`
- 获取指定 thread_id 的近期对话摘要
- 从 LangGraph checkpoint 读取

#### `claude_code_output(session_id)`
- 获取指定 Claude Code 会话的输出（tail 最后 N 行）

#### `stop_claude_session(session_id)`
- 强制终止 Claude Code tmux 会话（`tmux kill-session`）

### 5.6 钉钉 Pipeline 工具（4 个）
...
#### `analyze_and_save_meeting(doc_id)`
- 完整会议纪要处理 pipeline：读取 → 分析 → 写入飞书

#### `sync_context_to_feishu()`
- 将本地 SQLite checkpoints 快照推送到飞书上下文页
...
- `_chat_locks` 字典，每个 `chat_id` 一把锁
- 同 chat 下不同话题**串行**处理（话题内并发）
- **注意**：锁粒度是 chat，非 thread（不同话题不可并行）
...
```python
_chat_locks: dict[str, threading.Lock]  # chat_id → Lock
```

- 同一 `chat_id` 的消息串行处理（等待前一条处理完毕）
- 防止乱序处理和重复响应
- 不同 `chat_id` 独立并发
...
| `ToolMessage` | 工具执行结果 | 历史 ToolMessage 内容截断至 300 字符 |
...
- `MAX_TOOL_ITERATIONS=10`（`graph/nodes.py`）
- 超限时强制终止并通知用户
- 防止 LLM 陷入无限工具调用循环
...
*本文档根据 v1.0.17 代码自动分析生成，最后更新：2026-03-22*
