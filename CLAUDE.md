...
✅ 定时任务        — 钉钉会议30分钟/邮件60分钟/上下文同步30分钟/心跳30分钟/每日迁移08:00
...
**触发**：用户反馈在处理不相关多任务时出现上下文污染（A项目的工具结果混入B项目的回复）。

**修改文件**：
- `graph/nodes.py` — `_trim_to_user_turns()` 新增步骤2：对非当前轮的 ToolMessage 内容截断至 `HISTORY_TOOL_CONTENT_LIMIT=300` 字符，防止旧任务工具结果（飞书页面/钉钉文档全文）污染新任务上下文；日志记录截断数量
- `prompts/system.md` — 新增"跨任务上下文隔离"节：明确指示 LLM 以当前用户消息为准，忽略历史工具结果中与当前任务无关的内容
- `workspace/MEMORY.md` — 基于49条交互分析更新：token均值76.9K、错误率22%、多任务污染根因；新增v0.8.20改进记录
- `workspace/HEARTBEAT.md` — 上下文健康检查新增 token 监控基准（v0.8.20 后预期 avg <50K）

**根因分析**：
- 用户在单一飞书对话 thread 中连续处理多个不相关项目（如"知识库项目会议纪要"→"VOC项目进展"）
- `_trim_to_user_turns` 只截断消息数量，未处理历史 ToolMessage 内容；飞书/钉钉文档全文（数千字符）残留在 5轮历史中，LLM 误以为是当前任务背景
- 平均 76.9K input tokens，82% 超30K，主要来源是 ToolMessage 内容积累

**效果预期**：
- 历史 ToolMessage 截断至300字符，大幅减少非当前任务的上下文噪声
- input tokens 预期从 76.9K 降至 <50K（历史5轮×文档内容 → 历史4轮×300字符）
- LLM 收到明确隔离指令，不再混用不同任务的工具结果

### v0.8.19（2026-03-20）— 会议纪要飞书展示样式优化：富 Markdown + 富文本块写入

**修改文件**：
- `integrations/meeting/analyzer.py` — 重写 `format_for_feishu` 和 `format_for_project_page` 两个格式化函数；`write_to_feishu` 和 `write_to_project_page` 升级为使用 `append_blocks_to_page` + `md_to_feishu_blocks` 写入，纯文本 `append_to_page` 保留为降级兜底
- `tests/regression/test_meeting_format.py` — **新建**：29 个单元测试，覆盖格式化函数所有分支 + 飞书块兼容性

**样式优化内容**：
1. **会议基本信息表格化**：日期/参与人/原始文档改为 Markdown 表格（`|字段|内容|`），配 emoji 图标（📅👥🔗📁）
2. **摘要使用引用块**：`> 摘要内容` 替代原 `**摘要**: 内容`，在飞书渲染为高亮引用样式
3. **子标题层级化**：决策/待办/跟进改为 H3（`###`）+ emoji 前缀（✅📌🔄），原来是粗体标签
4. **待办事项样式增强**：负责人加粗 `**（张三）**`，截止日期用代码块 `` `2026-03-25` `` 突出显示
5. **项目页新增章节**：里程碑（🏁）、本周状态（📊）、详细决策（📜）、识别风险（⚠️）均升级为 H3 标题
6. **风险等级格式化**：概率/影响改为行内代码 `` `[高/高]` ``，更易扫读
7. **富文本块写入**：调用 `md_to_feishu_blocks` 转换后用 `append_blocks_to_page` 写入，标题/表格/引用/列表正确渲染；API 失败时自动降级纯文本

**能力变化**：
- 飞书会议纪要页面从纯文本渲染升级为结构化富文本（标题层级、信息表格、摘要引用块、加粗截止日期）
- 向后兼容：函数签名不变，调用方无需修改；API 调用失败时自动降级到原有纯文本路径
- 新增 29 个单元测试，无需飞书 token 即可本地运行
...
**修改文件**：
- `integrations/feishu/knowledge.py` — 更新 `_PROJECT_DOCS` 为敏捷结构（6个文档），更新 `_PROJECT_TEMPLATES`：新增 `01_需求清单`（用户故事）、`05_迭代计划`（Sprint）、`07_需求交付记录`；移除 `02_技术方案`、`03_项目计划`、`05_状态周报`；`06_RAID 日志`新增「需求验收」表格
- `graph/tools.py` — `feishu_project_setup` 父页面优先级调整：不再回落到 `FEISHU_WIKI_CONTEXT_PAGE`，未配置 `FEISHU_WIKI_PORTFOLIO_PAGE` 时直接在 wiki 根目录创建；更新 docstring 反映敏捷结构
- `integrations/meeting/project_router.py` — `_portfolio_root()` 移除 `FEISHU_WIKI_CONTEXT_PAGE` 回落，改为 wiki 根目录（space_id）；`route_meeting()` 改为创建每会议独立子页面：在 `04_会议纪要` 文件夹下创建 `YYYY-MM-DD 会议标题` 子页面；`PAGE_WEEKLY_REPORT` 改为 `05_迭代计划`；导入 `datetime`

**能力变化**：
- 新创建项目自动出现在 wiki 根目录（与首页齐平），而非 AI 助理上下文页面下
- 会议纪要按日期+标题拆分独立子页面（`04_会议纪要/YYYY-MM-DD 会议主题`），不再堆积单页
- 项目文档结构由瀑布式（7 文档）调整为敏捷业务型（6 文档）：业务侧文档 + Sprint 规划，移除技术文档
- RAID 日志新增「需求验收」维度，跟踪业务需求交付状态
- 迁移提示：现有项目文件夹需手动移动到 wiki 根目录；`agent_config` 设置 `FEISHU_WIKI_PORTFOLIO_PAGE` 可指定统一目录

**设计决策**：
- `FEISHU_WIKI_CONTEXT_PAGE` 是 AI 助理上下文专用页，项目文件夹不应放在其下
- 会议子页面不缓存（每次新建），`04_会议纪要` 文件夹本身缓存，保持性能
- 技术文档（API设计/代码规范）迁移至 GitHub，飞书仅保留业务文档

### v0.8.13（2026-03-20）— 每日会议迁移插件 + 飞书富文本格式 + 原始时间保留

**修改文件**：
- `integrations/feishu/rich_text.py` — **新建**：Markdown → 飞书 docx 块转换器，支持标题/加粗/斜体/列表/代码块/分割线等块级元素
- `integrations/feishu/knowledge.py` — 新增 `append_blocks_to_page()`：接受块列表写入飞书（替代纯文本 `append_to_page()`）
- `integrations/meeting/daily_migration.py` — **新建**：`DailyMigrationPlugin` 插件，独立 SQLite 状态（`data/daily_migration.db`），提取原始钉钉时间，调用富文本写入
- `integrations/meeting/analyzer.py` — `format_for_project_page()` 新增 `doc_time` 参数，显示原始文档时间而非迁移时间
- `integrations/dingtalk/docs.py` — `_normalize_node()` 新增 `created_at` 字段（从 `createTime`/`createdTime` 提取）
- `scheduler.py` — 新增 `daily_meeting_migration()` + `cron` job（每天 08:00，可通过 `DAILY_MIGRATION_RUN_HOUR` 配置）
- `graph/tools.py` — 新增 `trigger_daily_migration` / `list_daily_migrations` 工具，并入 `dingtalk_mcp` 分类

**能力变化**：
- 每天 08:00 自动检查钉钉新会议纪要并以富文本格式迁移到飞书（无需手动触发）
- 迁移后飞书页面显示**原始钉钉文档时间**（而非迁移时间），`_normalize_node` 同步暴露 `created_at`
- 飞书页面格式升级：标题 / 加粗 / 斜体 / 列表项 / 代码块 / 分割线，不再显示裸 Markdown 符号
- 插件可通过 `agent_config` 配置：`DAILY_MIGRATION_LOOKBACK_DAYS`（回溯天数，默认7）、`DAILY_MIGRATION_PROJECT_MAP`（手动项目映射 JSON）、`DAILY_MIGRATION_RUN_HOUR`（执行小时，默认8）
- 用户可通过 IM 说"手动触发今日迁移"→ Agent 调用 `trigger_daily_migration` 工具立即执行

**设计决策**：
- 每日迁移插件独立于 30 分钟轮询，有自己的去重状态（`data/daily_migration.db`），两套流程互不干扰
- 30 分钟轮询继续以 Markdown 格式快速写入，每日插件提供"精装版"富文本格式
- 富文本转换使用 Feishu docx API 原生块结构（block_type 3-13/24），非 Markdown 文字渲染
...
- `feishu_project_setup` 工具：一句话创建完整敏捷项目结构（6 文档：章程/需求清单/会议纪要/迭代计划/RAID/交付记录）
...
| `feishu_project_setup` | 飞书知识库 | 一键创建敏捷项目文件夹结构（6文档：章程/需求清单/会议纪要/迭代计划/RAID/交付记录），默认建在 wiki 根目录，幂等，结果缓存到 config_store |
...
| `trigger_daily_migration` | 钉钉/会议Skill | 手动触发每日会议迁移（富文本格式，保留原始钉钉时间） |
| `list_daily_migrations` | 钉钉/会议Skill | 查看每日迁移插件已处理的文档列表 |
