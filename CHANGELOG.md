...
### Feature
- **主窗口短标题话题自动检测**：在主聊天窗口发送 `#标题`（总字数 < 10，无正文）时，自动检查已有话题：有完全相同话题则直接切换；有相近话题（子串匹配或字符集重叠 ≥ 60%）则提示用户确认是否合并；无匹配则创建新话题并确认。`topic_manager.py`：新增 `find_similar_topics()`；`bot.py`：在话题解析块前插入短标题检测逻辑。

### Fixed
- **修复话题工具循环**：之前用户发送话题功能配置请求时，Agent 误以为要通过工具实时执行（manage_topics + python_execute 循环），触发循环检测。根因是该需求需代码层实现，本次在 bot.py 中直接实现避免了 LLM 工具调用路径。
