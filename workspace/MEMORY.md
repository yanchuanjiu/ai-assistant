...
_2026-03-22（基于 171 条交互日志分析，v1.0.9 自我改进）_
...
- **token 消耗极重（⚠️ 持续未解决）**：LLM 990次调用，avg input 43,640 tokens（超30K阈值46%的调用），主飞书线程 650次调用 avg 55,883 tokens，max 单次 203K。根因：完整历史不截断 + 工具调用结果大 + 系统 prompt 随关键词动态增长。
- **话题路由 BUG（⚠️ 3月22日新发现）**：用户在聊天框内回复某话题线程，未被正确路由到对应话题上下文。现象："我回复的这条信息没有加入到 话题 #实时讯息查询"。根因待分析：可能是飞书 root_id 反向映射缺失或聊天框内 reply 的消息未携带 root_id。
